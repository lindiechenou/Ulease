...
type Props = {
    roommates:number
    roommatesModal:boolean
    callbackModal:Function
    callbackCancelModal:Function
  };

export default function RoommatesModal(props:Props){
    const styles= useStyleSheet(themedStyles)
    const [roommates, setRoommates] = useState<string>(props.roommates.toString())

    const cancel = ( () => {
        setRoommates(props.roommates.toString())
        props.callbackCancelModal(RoommatesModal)
    }
    )
    useEffect(() => {
      setRoommates(props.roommates.toString()) 
              ...
              <KeyboardAvoidingView
                ...
                <View style={styles.modalView}>
                  <View style={styles.backgroundHeader}>
                    <Text category='s1' style={styles.headerText}>Edit Number of Roommates</Text>
                    </View>
                    <Input style={styles.inputView}
                         label={evaProps => <Text {...evaProps}>Enter Value</Text>}
                    ...
                    <Button style={styles.button}
                        size='small'
                        onPress={() => props.callbackModal(Number(roommates))}>
                        submit
                    </Button>
                    <Button
                    onPress={() => cancel()}
